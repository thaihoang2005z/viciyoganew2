      ...
      if (phoneMatch && !leadSaved) {
        ...
        if (!detectedName) {
          ...
          const cleanBefore = beforePhone
            .replace(/^(mình\s+tên\s+là|em\s+tên\s+là|tên\s+mình\s+là|tôi\s+tên\s+là|mình\s+là|em\s+là|tôi\s+là|tên:|sđt:|sđt\s+là|tên\s+học\s+viên\s+là|họ\s+tên\s+là)\s*/i, '')
            .trim();
          if (cleanBefore.length >= 2 && cleanBefore.length <= 30 && !cleanBefore.toLowerCase().includes('yoga') && !cleanBefore.toLowerCase().includes('khóa') && !cleanBefore.toLowerCase().includes('chào')) {
