...
export default function AdminDashboard({ onClose, onLogout }: AdminDashboardProps) {
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-[#F8F5EE] ${isAnySubModalOpen ? 'overflow-hidden' : 'overflow-y-auto'}`}>
      <div className="min-h-screen bg-[#F8F5EE] flex flex-col">
        {/* Top Navbar */}
        <header className="bg-[#21241E] text-white px-3 sm:px-8 py-3 sm:py-4 border-b border-[#3A3E34] sticky top-0 z-30 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          {/* Title & Back Button */}
          <div className="flex items-center justify-between sm:justify-start gap-2.5 w-full sm:w-auto">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h1 className="text-sm sm:text-lg font-bold font-serif-display">
                    VICI Lead CRM
                  </h1>
                  <span className="text-[10px] sm:text-[11px] px-2 py-0.5 rounded bg-[#D69A2D] text-white font-semibold whitespace-nowrap">
                    Hệ Thống Phụ Trách
                  </span>
                </div>
            ...
            {/* Close Button for Mobile Right Corner */}
            <button
              onClick={onClose}
              className="sm:hidden px-3 py-1.5 rounded-xl bg-[#D69A2D] hover:bg-[#B87A14] text-xs font-bold text-white transition-all cursor-pointer whitespace-nowrap shrink-0"
            >
              Về Web
            </button>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5 overflow-x-auto pb-1 sm:pb-0 scrollbar-none w-full sm:w-auto">
            {/* Firebase Live Cloud Sync Indicator */}
            <div
              className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border text-[11px] sm:text-xs font-medium transition-all shrink-0 whitespace-nowrap ${
              ...
              title="Đồng bộ tự động thời gian thực qua Firebase Firestore giữa mọi thiết bị"
            >
              <span
              ...
              <span className="md:hidden">
                {firebaseStatus === 'synced' ? 'Live' : 'Offline'}
              </span>
            </div>
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-xl bg-gradient-to-r from-amber-700/80 to-[#8A6437] hover:from-amber-600 hover:to-[#6F4E27] border border-amber-400/40 text-xs font-bold text-amber-50 transition-all cursor-pointer shadow-2xs whitespace-nowrap shrink-0"
              title="Sao lưu và đồng bộ toàn bộ hội thoại và hồ sơ khách hàng dạng JSON"
            >
              <Database className="w-3.5 h-3.5 text-amber-200 shrink-0" />
              <span>Sao Lưu</span>
            </button>
            ...
            <button
              ...
              className="hidden lg:inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-xs font-semibold text-stone-200 transition-all cursor-pointer whitespace-nowrap shrink-0"
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-xl bg-gradient-to-r from-[#D69A2D] to-[#B87A14] hover:from-[#E5A83B] hover:to-[#A36A0E] text-xs font-bold text-white transition-all cursor-pointer shadow-xs whitespace-nowrap shrink-0"
            ...
            {onLogout && (
              <button
                onClick={onLogout}
                className="inline-flex items-center gap-1 px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-xl bg-red-950/40 hover:bg-red-900/70 border border-red-500/30 text-xs font-semibold text-red-200 transition-all cursor-pointer whitespace-nowrap shrink-0"
            ...
            <button
              onClick={onClose}
              className="hidden sm:inline-block px-3.5 py-2 rounded-xl bg-[#D69A2D] hover:bg-[#B87A14] text-xs font-bold text-white transition-all cursor-pointer whitespace-nowrap shrink-0"
            >
              Về Website
            </button>
          </div>
        ...
        {selectedLead && createPortal(
          <div
            className="fixed inset-0 z-[9999] w-full h-full flex items-center justify-center p-3 sm:p-6 bg-black/80 backdrop-blur-md overflow-y-auto animate-in fade-in duration-200"
            onClick={(e) => {
          ...
          >
            <div className="bg-[#FFFDF8] w-full max-w-2xl rounded-3xl shadow-2xl border border-[#E8DFC8] overflow-hidden my-auto max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-200">
              {/* Modal Header */}
                ...
                {selectedLead.aiReport?.fullSummaryText ? (
                  <div className="p-4 rounded-2xl bg-[#FAF7F0] border border-[#E8DFC8] space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] bg-[#D69A2D]/15 text-[#8A6437] font-semibold px-2 py-0.5 rounded-full border border-[#D69A2D]/30">
                        Tương Tác MyVici AI Chatbot
                      </span>
                    </div>
        ...
        {leadToDelete && createPortal(
          ...
          >
            <div className="bg-[#FFFDF8] w-full max-w-md rounded-3xl shadow-2xl border border-red-200 overflow-hidden my-auto animate-in zoom-in-95 duration-200">
